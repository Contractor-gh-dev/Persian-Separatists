...
public class CodexRadiation extends CodexEntryV2 implements CustomUIPanelPlugin {
	protected CustomPanelAPI panel;
	protected UIPanelAPI relatedEntries;
	protected UIPanelAPI box;
	protected CodexDialogAPI codex;

	public CodexRadiation(String id, String title, String icon) {
		super(id, title, icon);
	}
	...
	public void createCustomDetail(CustomPanelAPI panel, UIPanelAPI relatedEntries, CodexDialogAPI codex) {
		...
		text.addPara("The Radioactive Decay status effect causes ships to lose CR faster.", opad);
		opad = 10f;
		LabelAPI label1 = text.addPara("Affected ships lose CR %s faster, and will lose the extra amount even when %s is affecting them. The effect will last " +
						"approximately %s to the length of time as the ship spent inside the radiation zone.", opad, h,
				"50%", "no enemy presence", "equal");
		label1.setHighlightColors(b, h, h);
		label1.setHighlight("50%", "no enemy presence", "equal");
