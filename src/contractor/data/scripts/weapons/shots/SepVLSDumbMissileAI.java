...
public class SepVLSDumbMissileAI implements MissileAIPlugin, GuidedMissileAI {
	...
	private float timer = 0, check = 0f, initialTimer = 0f;
	...
	public SepVLSDumbMissileAI(MissileAPI missile, ShipAPI launchingShip) {
	...
	public void advance(float amount) {
		...
		if (target == null
			...
			initialTimer = 0f;
			if (MISSILE.getVelocity().length() < 125f)
		...
		if (!initial)
			correctAngle += ((float) (WAVE_AMPLITUDE * FastTrig.cos(OFFSET + MISSILE.getElapsed() * (2 * MathUtils.FPI / WAVE_TIME)))) //add wave
					* Math.min(1f, Math.max(0.3f, MathUtils.getDistanceSquared(MISSILE.getLocation(), target.getLocation()) / PRECISION_RANGE)); //range damper
		...
		if (initial) {
			if (MISSILE.getVelocity().length() < 125f)
				MISSILE.giveCommand(ShipCommand.ACCELERATE);
			if (Math.abs(aimAngle) < 75f) {
				initialTimer += amount;
				if (initialTimer >= 0.75f) {
					initial = false;
					TARGET_SWITCH = false;
				}
			}
		} else if (Math.abs(aimAngle) < 45f)
			MISSILE.giveCommand(ShipCommand.ACCELERATE);
