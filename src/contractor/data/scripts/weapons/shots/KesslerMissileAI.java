...
public class KesslerMissileAI implements MissileAIPlugin, GuidedMissileAI {
	...
	private final float WAVE_TIME = 1f;
	...
	private float ECCM = 2.5f;   //A VALUE BELOW 1 WILL PREVENT THE MISSILE FROM EVER HITTING ITS TARGET!
	...
	private boolean launch = true, fuelPop = true;
	private float timer = 0, check = 0f, fuel;
	private final java.awt.Color eColor = new Color(230, 90, 20, 200);
	private final java.awt.Color sColor = new Color(110, 100, 90);
	...
	public KesslerMissileAI(MissileAPI missile, ShipAPI launchingShip) {
		...
		fuel = missile.getMaxRange() / missile.getMaxSpeed();
	}
	...
	public void advance(float amount) {
		...
		if (fuelPop && fuel < 0f) {
			...
			for (int i = 0; i < 8; i++) {
				...
				float rand = (float) ((Math.random() - 0.5f) * 10f);
				Vector2f vel;
				...
				VectorUtils.resize(vel, 64f + (float) ((Math.random() - 0.5f) * 30f), vel);

				engine.addHitParticle(loc, vel, 10f + (float) ((Math.random() - 0.5f) * 8f), 0.3f, 0.5f, eColor);
			}
			for (int i = 0; i < 10; i++) {
				float face = MISSILE.getFacing() + 180f;
				float rand = (float) ((Math.random() - 0.5f) * 10f);
				...
				engine.addSmokeParticle(loc, vel, 10f + (float) ((Math.random() - 0.5f) * 10f), 0.4f, 0.75f, sColor);
			}
		}
		if (fuel < 0) {
			MISSILE.flameOut();
		...
		if (launch || timer >= check) {
			...
			check = Math.min(0.25f, Math.max(0.05f, MathUtils.getDistanceSquared(MISSILE.getLocation(), target.getLocation()) / PRECISION_RANGE)
			);
		...
		float correction = MathUtils.getShortestRotation(correctAngle, VectorUtils.getFacing(MISSILE.getVelocity()) + 180)
				* 0.5f * //oversteer
		...
		correctAngle += (float) (multiplier * WAVE_AMPLITUDE * check * Math.cos(OFFSET + MISSILE.getElapsed() * (2 * MathUtils.FPI / WAVE_TIME)));
