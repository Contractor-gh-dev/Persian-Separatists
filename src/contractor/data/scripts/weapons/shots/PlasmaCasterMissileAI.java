...
public class PlasmaCasterMissileAI implements MissileAIPlugin, GuidedMissileAI {
	//////////////////////
	//     SETTINGS     //
	//////////////////////
	...
	private final float WAVE_AMPLITUDE = 6;
	...
	private final int capitals = 3;

	//Arc to look for targets into
	//set to 360 or more to ignore
	private final int SEARCH_CONE = 330;
	...
	public PlasmaCasterMissileAI(MissileAPI missile, ShipAPI launchingShip) {
		...
		PRECISION_RANGE = (float) Math.pow((PRECISION_RANGE), 2);
	...
	public void advance(float amount) {
		...
		if (target == null
			...
			this.target = MagicTargeting.pickTarget(
					MISSILE,
					seeking,
					MAX_SEARCH_RANGE,
					SEARCH_CONE,
		...
		float offCourseAngle = MathUtils.getShortestRotation(VectorUtils.getFacing(MISSILE.getVelocity()), correctAngle);

		float correction = MathUtils.getShortestRotation(correctAngle, VectorUtils.getFacing(MISSILE.getVelocity()) + 180) * 0.5f * //oversteer
