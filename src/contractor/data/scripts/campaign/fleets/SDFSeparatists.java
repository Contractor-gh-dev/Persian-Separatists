...
public class SDFSeparatists extends SDFBase {
	public SDFSeparatists() {
	}
	...
	protected MarketAPI getSourceMarket() {
		return Global.getSector().getEconomy().getMarket("separatists_sedna_market");
	}

	@Override
	protected String getDefeatTriggerToUse() {
		return "SDFSeparatists1Defeated";
	}
	...
	public boolean canSpawnFleetNow() {
		...
		if (!source.hasIndustry("sep_orbitalstation_medium")) return false;
	...
	public CampaignFleetAPI spawnFleet() {
		MarketAPI sedna = getSourceMarket();
		Vector2f loc = sedna.getLocationInHyperspace();
		...
		m.triggerSetFleetSizeFraction(1f);
		...
		m.triggerSetPatrol();
		m.triggerSetFleetMemoryValue(MemFlags.MEMORY_KEY_SOURCE_MARKET, sedna);
		m.triggerFleetSetName("Star Navy 1st Battalion");
		m.triggerPatrolAllowTransponderOff();
		m.triggerOrderFleetPatrol(sedna.getStarSystem());

		CampaignFleetAPI fleet = m.createFleet();
		fleet.removeScriptsOfClass(MissionFleetAutoDespawn.class);
		sedna.getContainingLocation().addEntity(fleet);
		fleet.setLocation(sedna.getPlanetEntity().getLocation().x, sedna.getPlanetEntity().getLocation().y);
